<?php
  ...
  "July", "August", "September", "October", "November", "December");
	   ...
	   if ( $last_order_time_diff) $count= $last_order_time_diff; else $count=__( '0', 'icwoocommerce_textdomains');
		...
		$today_summary.= $this->pw_get_dashboard_boxes_generator($type, 'pink-2', 'basket', __('Todays Registered Customers',__PW_REPORT_WCREPORT_TEXTDOMAIN__), $today_reg_customer, 'price', '', 'number',$progress_html);
		
		$progress_html= $this->pw_get_dashboard_progress_contents($today_guest_customer,$yesterday_guest_customer);
		$today_summary.= $this->pw_get_dashboard_boxes_generator($type, 'orange-2', 'chart', __('Todays Guest Customers',__PW_REPORT_WCREPORT_TEXTDOMAIN__), $today_guest_customer, 'price', '', 'number',$progress_html);
