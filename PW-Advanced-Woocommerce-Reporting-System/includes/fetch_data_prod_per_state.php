<?php
	...
	if($file_used=="sql_table")
	{
		...
		$sql = "SELECT $sql_columns FROM $sql_joins $category_id_join $pw_id_order_status_join WHERE $sql_condition  $pw_from_date_condition $category_id_condition
		...
		if($state_code != NULL  && $state_code != '-1')
		{
			...
			foreach($state_codes as $state){
				...
				{
					$data_state[]=$state -> id;
					$value=array(array('lable'=>$state -> label,'status'=>'show'));
