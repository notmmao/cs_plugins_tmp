<?php
	...
	if($file_used=="sql_table")
	{
		...
		if($pw_product_stock || $pw_txt_min_stock || $pw_txt_max_stock || strlen($pw_product_stock) >0 || $pw_zero_stock == "yes") 		$pw_product_stock_join = " LEFT JOIN  {$wpdb->prefix}postmeta as pw_stock 				ON pw_stock.post_id			=pw_posts.ID";
		...
		if($pw_product_stock || $pw_txt_min_stock || $pw_txt_max_stock || strlen($pw_product_stock) >0 || $pw_zero_stock == "yes") 		$pw_product_stock_condition_1 = " AND pw_stock.meta_key ='_stock'";
		...
		//for($i=1; $i<=20 ; $i++){
			$datatable_value.=("<tr>");
				...
				if($pw_basic_column=='yes'){
					...
					if($this->table_cols[$j++]['status']=='hide') $display_class='display:none';
					$datatable_value.=("<td style='".$display_class."'>");
						$datatable_value.= $product_details['stock']!='' ? round($product_details['stock']) : "0";
					$datatable_value.=("</td>");
